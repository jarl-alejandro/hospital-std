...
doctor.controller('pacientesDoctorController', function ($scope, $http, $location) {
  ...
  $scope.form = object => {
    ...
    if (duracion.months < 2 && duracion.years <= 0) $location.path(`/form28A/${object.id}/${object.turno}/save`)
    else if (duracion.months >= 2 && duracion.years <= 9) $location.path(`/form28C/${object.id}/${object.turno}/save`)
