	...
	return {
		...
		compile: function (element, attrs) {
			...
			return function (scope, element, atributos) {
				...
				var controls = angular.element('<div class="">'+
					'<button class="waves-effect waves-light btn primary" ng-click="'+registro.getBeforeNotation()+'()" type="button"'+
					'ng-disabled="'+registro.getCurrentNotation()+'<=0"><i class="material-icons">arrow_back</i></button>'+

					'<small class="indicator margin">{{'+registro.getCurrentNotation()+'+1}}/{{'+registro.getPagesNotation()+'}}</small>'+

					'<button class="waves-effect waves-light btn primary" ng-click="'+registro.getNextNotation()+'()" type="button"'+
					'ng-disabled="'+registro.getCurrentNotation()+'>=('+registro.getDataLengthNotation()+'/'+registro.getSizeNotation()+')-1"><i class="material-icons">arrow_forward</i></button>'+
				'</div>');
